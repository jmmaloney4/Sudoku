...
public class UIMgr {
	...
	public void Start() throws ClassNotFoundException, InstantiationException,
		...
		frame.setBounds((dim.width / 2) - (frame.getSize().width / 2),
				(dim.height / 2) - (frame.getSize().height / 2), 700, 175);
		frame.setBounds((dim.width / 2) - (frame.getSize().width / 2),
				(dim.height / 2) - (frame.getSize().height / 2), 700, 175);
		...
		npane.add(Box.createHorizontalStrut(150), BorderLayout.WEST);
		...
		spane.add(Box.createHorizontalStrut(560), BorderLayout.WEST);
